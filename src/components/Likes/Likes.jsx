...
function Likes({itemId}) {
  ...
  useEffect(
    ()=>{
        ...
        if(user){
            const q = query(likesRef, where("itemId", "==", itemId), where("userId", "==", user?.uid))
        ...
        // find the like count 
        const q2 = query(likesRef, where("itemId", "==", itemId))
        getDocs(q2, likesRef)
        .then(res => {
  ...
  const handleLike = () => {
    ...
    if(user){
        ...
        addDoc(likesRef, {userId: user?.uid, itemId: itemId})
        .then(res=> {
            //see that its liked
            setIsLiked(true)
        })
